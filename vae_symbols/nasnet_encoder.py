...
class CellStem0(nn.HybridBlock):
    ...
    def __init__(self):
        ...
        self.conv_1x1.add(nn.Conv2D(21, 1, strides=1, use_bias=False))
        self.conv_1x1.add(nn.BatchNorm(epsilon=0.001, momentum=0.1))

        self.comb_iter_0_left = BranchSeparables(21, 21, 5, 2, 2)
        self.comb_iter_0_right = BranchSeparablesStem(48, 21, 7, 2, 3, bias=False)

        self.comb_iter_1_left = nn.MaxPool2D(pool_size=3, strides=2, padding=1)
        self.comb_iter_1_right = BranchSeparablesStem(48, 21, 7, 2, 3, bias=False)

        self.comb_iter_2_left = nn.AvgPool2D(pool_size=3, strides=2, padding=1)
        self.comb_iter_2_right = BranchSeparablesStem(48, 21, 5, 2, 2, bias=False)

        self.comb_iter_3_right = nn.AvgPool2D(pool_size=3, strides=1, padding=1)

        self.comb_iter_4_left = BranchSeparables(21, 21, 3, 1, 1, bias=False)
...
class CellStem1(nn.HybridBlock):
    ...
    def __init__(self):
        ...
        self.conv_1x1.add(nn.Conv2D(channels=42, kernel_size=1, strides=1, use_bias=False))
        self.conv_1x1.add(nn.BatchNorm(epsilon=0.001, momentum=0.1))

        self.relu = nn.Activation('relu')
        self.path_1 = nn.HybridSequential()
        self.path_1.add(nn.AvgPool2D(pool_size=1, strides=2))
        self.path_1.add(nn.Conv2D(channels=21, kernel_size=1, strides=1, use_bias=False))

        #self.path_2 = nn.ModuleList()
        #self.path_2.add_module('pad', nn.ZeroPad2d((0, 1, 0, 1)))
        #self.path_2.add_module('avgpool', nn.AvgPool2d(1, stride=2, count_include_pad=False))
        self.path_2_avgpool = nn.AvgPool2D(pool_size=1, strides=2)
        self.path_2_conv = nn.Conv2D(channels=21, kernel_size=1, strides=1, use_bias=False)

        self.final_path_bn = nn.BatchNorm(epsilon=0.001, momentum=0.1)

        self.comb_iter_0_left = BranchSeparables(42, 42, 5, 2, 2, bias=False)
        self.comb_iter_0_right = BranchSeparables(42, 42, 7, 2, 3, bias=False)

        self.comb_iter_1_left = nn.MaxPool2D(pool_size=3, strides=2, padding=1)
        self.comb_iter_1_right = BranchSeparables(42, 42, 7, 2, 3, bias=False)

        self.comb_iter_2_left = nn.AvgPool2D(pool_size=3, strides=2, padding=1)
        self.comb_iter_2_right = BranchSeparables(42, 42, 5, 2, 2, bias=False)
...
class NASNetALarge(nn.HybridBlock):
    ...
    def __init__(self, encoder_args):
        ...
        fc_hidden = encoder_args['fc_hidden']

        self.conv0 = nn.HybridSequential()
        self.conv0.add(nn.Conv2D(channels=48, kernel_size=3, padding=1, strides=1, use_bias=False))
        self.conv0.add(nn.BatchNorm(epsilon=0.001, momentum=0.1))

        self.cell_stem_0 = CellStem0()
        self.cell_stem_1 = CellStem1()

        self.cell_0 = FirstCell(in_channels_left=84, out_channels_left=42,
                                in_channels_right=168, out_channels_right=84)
        self.cell_1 = NormalCell(in_channels_left=168, out_channels_left=84,
                                 in_channels_right=504, out_channels_right=84)
        self.cell_2 = NormalCell(in_channels_left=504, out_channels_left=84,
                                 in_channels_right=504, out_channels_right=84)
        self.cell_3 = NormalCell(in_channels_left=504, out_channels_left=84,
                                 in_channels_right=504, out_channels_right=84)
        """self.cell_4 = NormalCell(in_channels_left=504, out_channels_left=84,
                                 in_channels_right=504, out_channels_right=84)
        self.cell_5 = NormalCell(in_channels_left=504, out_channels_left=84,
                                 in_channels_right=504, out_channels_right=84)
                                 """

        self.reduction_cell_0 = ReductionCell0(in_channels_left=504, out_channels_left=168,
                                               in_channels_right=504, out_channels_right=168)

        self.cell_6 = FirstCell(in_channels_left=504, out_channels_left=84,
                                in_channels_right=672, out_channels_right=168)
        self.cell_7 = NormalCell(in_channels_left=672, out_channels_left=168,
                                 in_channels_right=1008, out_channels_right=168)
        self.cell_8 = NormalCell(in_channels_left=1008, out_channels_left=168,
                                 in_channels_right=1008, out_channels_right=168)
        self.cell_9 = NormalCell(in_channels_left=1008, out_channels_left=168,
                                 in_channels_right=1008, out_channels_right=168)
        """
        self.cell_10 = NormalCell(in_channels_left=1008, out_channels_left=168,
                                  in_channels_right=1008, out_channels_right=168)
        self.cell_11 = NormalCell(in_channels_left=1008, out_channels_left=168,
                                  in_channels_right=1008, out_channels_right=168)
        """

        self.reduction_cell_1 = ReductionCell1(in_channels_left=1008, out_channels_left=336,
                                               in_channels_right=1008, out_channels_right=336)

        self.cell_12 = FirstCell(in_channels_left=1008, out_channels_left=168,
                                 in_channels_right=1344, out_channels_right=336)
        self.cell_13 = NormalCell(in_channels_left=1344, out_channels_left=336,
                                  in_channels_right=2016, out_channels_right=336)
        self.cell_14 = NormalCell(in_channels_left=2016, out_channels_left=336,
                                  in_channels_right=2016, out_channels_right=336)
        self.cell_15 = NormalCell(in_channels_left=2016, out_channels_left=336,
                                  in_channels_right=2016, out_channels_right=336)
        """
        self.cell_16 = NormalCell(in_channels_left=2016, out_channels_left=336,
                                  in_channels_right=2016, out_channels_right=336)
        self.cell_17 = NormalCell(in_channels_left=2016, out_channels_left=336,
                                  in_channels_right=2016, out_channels_right=336)
        """

        self.relu = nn.Activation(activation='relu')
        self.avgpool = nn.AvgPool2D(pool_size=8, strides=1, padding=0)
        self.flatten = nn.Flatten()
        self.dropout = nn.Dropout(0.25)
        self.dense = nn.Dense(fc_hidden)
    ...
    def features(self, x):
        ...
        x_cell_6 = self.cell_6(x_reduction_cell_0, x_cell_2)
